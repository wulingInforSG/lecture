...
\section{練習題}
\begin{itemize}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1001}{\underline{WLOJ 1001: Hello World!}}
\end{itemize}
...
\section{練習題}
\begin{itemize}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1002}{\underline{WLOJ 1002: A+B Problem}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1003}{\underline{WLOJ 1003: 球球的表面積}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1004}{\underline{WLOJ 1004: 我要成為化學大師}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1006}{\underline{WLOJ 1006: A+B Problem (Hard Version)}}
\end{itemize}
...
\section{練習題}
\begin{itemize}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1005}{\underline{WLOJ 1005: 季節判斷}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1007}{\underline{WLOJ 1007: 3n+1 Problem (Easy Version)}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1008}{\underline{WLOJ 1008: 天將降大任於斯人也}}
\end{itemize}
\chapter{while 迴圈}
...
\section{練習題}
\begin{itemize}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1009}{\underline{WLOJ 1009: 3n+1 Problem (Hard Version)}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1010}{\underline{WLOJ 1010: 階乘問題}}
\item \href{http://3dfa7fed3c4e.ngrok.io/problem/1011}{\underline{WLOJ 1011: 武陵專用拖鞋}}
\end{itemize}
...
那有沒有其他方式可以讓陣列長度放變數呢？答案是有的，不過這是高階的寫法：
\begin{code}
...
\subsection{宣告}
二維陣列宣告的方式如下：``資料型態\textbf{ }變數名稱[ 長度一 ][ 長度二 ];"，像如果想要宣告一個資料型態為\texttt{int}且長度一為4、長度二則為3的A陣列，其程式碼就會像下面那樣。
\begin{code}
...
\subsection{存取}
再來是第3\~5行，先前兩種宣告方式都可以
1.讀入整行字串(第4行)。若用cin>>讀入，會讀到' ','\textbackslash{}t','\textbackslash{}n'為止。
2.存取字串中某個字元(第5行)。
3.存取整個字串(第6行)。在每個字串的最後，皆會有'\textbackslash0'作結。故輸出到'\textbackslash0'時，會停止輸出。
\subsection{初始化}
...
\end{minted}
\end{code}
在初始化時，可以char型態的陣列一樣以char分開儲存。但這樣太麻煩了，所以我們可以用雙引號(")將字串夾起來。在main function中的第4行是錯誤的，為什麼呢?這是因為字串會以'\textbackslash0'結尾，所以若用字元陣列宣告的字串長度必須加1。
\subsection{文字處理函式}
...
\end{minted}
\end{code}
在\autoref{code:randsrc}~中，我們可以發現rand是透過固定的運算產生下一個數字，而且亂數會有一個最大值\texttt{RAND\_MAX}。以這種方式產生的亂數稱為偽隨機亂數，但是一般來說，rand()函數對於一般的任務已經足夠了。
